import QtQuick 2.3
import QtQuick.Window 2.2
import QtQuick.Controls 1.4
import QtQuick.Layouts 1.2
import QtQuick.Controls.Styles 1.4
import QtQuick.Controls.Private 1.0
pragma Singleton
QtObject{
    property color brightColor:"#C51162"
    property color greyColor: "#E0E0E0"
    property color backgroundColor: "#FAFAFA"
    property color backgroundCardBox: "#EEEEEE"
    property color backgroundCard:"#FAFAFA"
    property int marginBetween:4
    property int marginTop: 5
    property int marginMicro: 2
    property color buttonColor: "#BA68C8"
    property color buttonColorDown: "#6A1B9A"
    property color textButtonColor: "#ffffff"
    property int minWidthInputBox: 300
    property int minHeightCartBox:250    
}
